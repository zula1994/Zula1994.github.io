...
async function generatePaginationPages(total, pageSize) {
    ...
    const paths = resolve('./pages/page')
    if (total > 0) {
        for (let i = 1; i < pagesNum + 1; i++) {
            const path = i === 1 ? '.vitepress/theme/components/Page.vue' : '../../.vitepress/theme/components/Page.vue'
            const page = `
---
page: true
...
`.trim()
    ...
    await fs.move(paths + '/page_1.md', resolve('./index.md'), { overwrite: true })
}
